...
def build_visa_dataloader(cfg, training, distributed=True, category='candle'):
    ...
    train_data = VISA(root='/kaggle/input/visa-ds/VisA/1cls', train=True, transform=transform, category=category,
                      resize=224, use_imagenet=True, select_random_image_from_imagenet=True,
                      shrink_factor=1)
    padded = VISA(root='/kaggle/input/visa-ds/VisA/1cls', train=True, transform=transform, category=category,
                  resize=224, use_imagenet=True, select_random_image_from_imagenet=True,
    ...
    if training:
        ...
        return data_loader
    dataset_main = VISA(root='/kaggle/input/visa-ds/VisA/1cls', train=False, transform=transform, category=category,
                        resize=224, use_imagenet=True, select_random_image_from_imagenet=True,
                        shrink_factor=1)
    dataset_shifted = VISA(root='/kaggle/input/visa-ds/VisA/1cls', train=False, transform=transform, category=category,
                           resize=224, use_imagenet=True, select_random_image_from_imagenet=True,
                           shrink_factor=0.9)
...
class VISA(data.Dataset):
    ...
    def __init__(self, root, train=True,
                 transform=None,
                 category='candle', resize=None, use_imagenet=False,
                 select_random_image_from_imagenet=False, shrink_factor=1, shuffle=False, ratio=1, pad_train=False):
        self.root = root
        self.transform = transform
        self.image_files = []
        self.category = category
        print("category VISA:", category)
        if train:
            self.image_files = glob(os.path.join(root, category, "train", "good", "*.JPG"))
        else:
            image_files = glob(os.path.join(root, category, "test", "*", "*.JPG"))
