...
from datasets.camelyon_dataset import build_camelyon_dataloader
...
from datasets.gta_dataset import build_gta_dataloader
...
def build(cfg, training, distributed):
    ...
    if dataset == "custom":
        ...
        data_loader = build_gta_dataloader(cfg, training, distributed)
    else:
...
def build_dataloader(cfg_dataset, distributed=True):
    ...
    if cfg_dataset.get('type', None) in ['waterbirds', 'brain', 'isic', 'aptos', 'camelyon', 'gta']:
